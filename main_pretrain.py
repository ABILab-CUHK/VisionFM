# this code is modified from https://github.com/bytedance/ibot/tree/main
import argparse
import os
import sys
import datetime
...
from loader import PretrainMask
from utils import eval_pred
import wandb


def get_args_parser():
    parser = argparse.ArgumentParser('VisionFM pretraining code', add_help=False)

    # Model parameters
    parser.add_argument('--arch', default='vit_base', type=str,
                        choices=['vit_tiny', 'vit_small', 'vit_base', 'vit_large', ],
                        help="""Name of architecture to train. For quick experiments with ViTs""")
    parser.add_argument('--name', type=str, default=None, required=True, help='the trial name recorded in the wandb')
    parser.add_argument('--load_pretrain', action='store_true', help='whether to load the pretrain weights, will defaultly load the dino wights')
    parser.add_argument('--use_wandb', action='store_true', help='whether to the wandb to record the training logs')
    parser.add_argument('--pretrain_path', default=None, type=str, help='The user-defined path of pretrain weights')

    parser.add_argument('--input_size', type=int, default=224, help='input size')
    parser.add_argument('--modality', type=str, default='Fundus', help='modality to be trained')
    parser.add_argument('--patch_size', default=16, type=int, help="""Size in pixels of input square patches - default 16 (for 16x16 patches). """)
    parser.add_argument('--out_dim', default=8192, type=int, help="""Dimensionality of output for [CLS] token.""")
    parser.add_argument('--patch_out_dim', default=8192, type=int, help="""Dimensionality of output for patch tokens.""")
    ...
    parser.add_argument("--lr", default=0.000075, type=float, help="""Learning rate at the end of
        linear warmup (highest LR used during training). The learning rate is linearly scaled
        with the batch size, and specified here for a reference batch size of 256.""")
    parser.add_argument("--warmup_epochs", default=50, type=int,
                        help="Number of epochs for the linear learning-rate warm up.")
    parser.add_argument('--min_lr', type=float, default=2e-6, help="""Target LR at the
    ...
    parser.add_argument('--use_flash', action='store_true', help="""use the flash attention""")
    parser.add_argument('--scheme', type=str, default='single', choices=['single', 'mix'],
                        help="""the training scheme""")
    ...
    parser.add_argument('--global_crops_scale', type=float, nargs='+', default=(0.32, 1.),
                        help="""Scale range of the cropped image before resizing, relatively to the original image.
...
def train_encoder(args):
    ...
    transform = DataAugmentationiBOT(
        ...
        args.input_size,  # the img size of input
        modality=args.modality,
        scheme=args.scheme,
    )
    pred_size =  args.patch_size
    dataset = PretrainMask(
        root=args.data_path,
        transform=transform,
        ...
        modality=args.modality,  # the used dataset
    ...
    if args.arch in models.__dict__.keys():
        student = models.__dict__[args.arch](
            ...
            masked_im_modeling=args.use_masked_im_modeling, use_flash=args.use_flash,
        )
        teacher = models.__dict__[args.arch](
            img_size=[args.input_size],
            patch_size=args.patch_size,
            return_all_tokens=True, use_flash=args.use_flash,
        )
        embed_dim = student.embed_dim
        # load pretrain
        if args.load_pretrain:
            print('load pretrain models.')
            pretrained_weights = 'download' if args.pretrain_path is None else args.pretrain_path
            utils.load_pretrained_weights(student, pretrained_weights, 'student', args.arch, args.patch_size)
            utils.load_pretrained_weights(teacher, pretrained_weights, 'teacher', args.arch, args.patch_size)
    # otherwise, we check if the architecture is in torchvision models
        ...
        print(f"Unsupport architecture: {args.arch}")
    ...
    if utils.is_main_process():  # Tensorboard configuration
        ...
        if args.use_wandb and not sys.gettrace(): # if Ture, will use the wandb to record the logs
            wandb_logger = wandb.init(project="VisionFM", config=args.__dict__, name=args.name, dir=args.output_dir, job_type="train")
            print("init the wandb logger.")
        else:
            print(f"will not use the wandb.")
            wandb_logger = None
        ...
        if utils.is_main_process():
            with (Path(args.output_dir) / "log.txt").open("a") as f:
                ...
                for k, v in train_stats.items():
                    writer.add_scalar(k, v, epoch)  # write into the tensorboard
            # write the wandb logger
            if wandb_logger is not None:
                wandb_logger.log(log_stats)

    if utils.is_main_process() and wandb_logger is not None:
        wandb_logger.finish()  # close the wandb
...
def train_one_epoch(student, teacher, teacher_without_ddp, ibot_loss, data_loader,
    ...
    for it, (images, labels, masks) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        # update weight decay and learning rate according to their schedule # images: [[B, 3, 224, 224],], labels: [B], masks:[[B, 14, 14]]
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            teacher_output = teacher(
                images[:args.global_crops_number])  # return cls: [2B, 8192], patch: [2B, 196, 8192], after head
        ...
        pred1 = utils.concat_all_gather(probs1[0].max(dim=1)[1])  # x1 from teacher
        pred2 = utils.concat_all_gather(probs2[1].max(dim=1)[1])  # x2 from student
        acc = (pred1 == pred2).sum() / pred1.size(0)
        pred_labels.append(pred1)
        real_labels.append(utils.concat_all_gather(labels.to(pred1.device)))  # the class labels for loaded dataset
    ...
    # our datasets dont contain the labels, so these codes are useless
    pred_labels = torch.cat(pred_labels).cpu().detach().numpy()
    real_labels = torch.cat(real_labels).cpu().detach().numpy()
    # delete the element with the value is -1
    pos_idx = np.nonzero(real_labels == -1)[0]
    flag_acc = False
    if len(pos_idx) < len(pred_labels):
        flag_acc = True
        pred_labels = np.delete(pred_labels, pos_idx)
        real_labels = np.delete(real_labels, pos_idx)
...
class iBOTLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, student_local_cls, student_mask, epoch):
        ...
        student_cls_c = student_cls.chunk(self.ncrops)  # divided into two
        ...
        teacher_cls_c = teacher_cls_c.detach().chunk(self.ngcrops)  # stop gradient
        ...
        for q in range(len(teacher_cls_c)):
            for v in range(len(student_cls_c)):
                if v == q:
                    loss2 = torch.sum(-teacher_patch_c[q] * F.log_softmax(student_patch_c[v], dim=-1),
                                      dim=-1)  # reconstruct loss
...
class DataAugmentationiBOT(object):
    def __init__(self, global_crops_scale, local_crops_scale, global_crops_number, local_crops_number,
                 img_size=224, modality='Fundus', scheme='single'):
        ...
        key = modality
        # mean, std = stats[key]
        mean, std = utils.get_stats(key)
        print(f"Use {key} mean: {mean} and std: {std}")
        if scheme == 'single':
            ...
            print(f"Use the mix modalities for training")
            normalize = transforms.Compose([
                transforms.ToTensor(),
                # NormalizeMix(), # for mix scheme
            ])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('VisionFM', parents=[get_args_parser()])
